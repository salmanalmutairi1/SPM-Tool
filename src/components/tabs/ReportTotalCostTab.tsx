...
export default function ReportTotalCostTab() {
  ...
  return (
    <section>
      <div className="mb-3">
        <h2 className="text-base font-semibold text-gray-900">Report: Total Cost for Whole Project</h2>
          ...
          <tbody>
            {rows.map((row, index) => (
              <tr key={row.id} className="hover:bg-gray-50">
                ...
                <td className={tdCls}>{money(taskCosts[index])}</td>
              </tr>
            ))}
            {rows.length === 0 && (
              <tr>
            ...
            <tr className="bg-gray-100 font-semibold">
              ...
              <td className={tdCls} />
              <td className={tdCls} />
              <td className={`${tdCls} font-bold`}>{money(projectTotal)}</td>
            </tr>
          </tbody>
        </table>
      </div>
